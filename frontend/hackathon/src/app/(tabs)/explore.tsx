...
export default function MarkAttendanceScreen() {
  ...
  const [livenessSession, setLivenessSession] = useState<string | null>(null);
  ...
  const runLivenessCheck = useCallback(async () => {
    ...
    while (attempts < maxAttempts) {
      ...
      const result = await faceRecognitionService.checkLivenessWithBase64(
        base64,
        session ?? undefined,
      );
      if (!result) {
  ...
  if (!permission) {
    return <View style={styles.center}><Text>Loading Camera engine...</Text></View>;
  }

  if (!permission.granted) {
    return (
      <View style={styles.center}>
        <Text style={styles.permissionText}>We need your permission to use the camera for face recognition.</Text>
      ...
      <View style={styles.overlayContainer}>
        {stage === 'idle' && (
          <>
            ...
            <TouchableOpacity style={styles.startButton} onPress={runLivenessCheck}>
