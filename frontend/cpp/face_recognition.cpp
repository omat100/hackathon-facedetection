...
void FaceRecognitionEngine::ensure_models() {
    ...
    std::string sface_path = model_dir_ + "/face_recognition_sface_2021dec.onnx";

    if (!file_exists(yunet_path)) {
        std::cerr << "YuNet model not found at " << yunet_path << std::endl;
        return;
    }

    detector_ = cv::FaceDetectorYN::create(yunet_path, "", cv::Size(640, 640), 0.5f, 0.3f, 5000);

    if (!file_exists(sface_path)) {
        std::cerr << "Recognition model not found at " << sface_path << std::endl;
        return;
    }

    arcface_net_ = cv::dnn::readNetFromONNX(sface_path);
    if (arcface_net_.empty()) {
        std::cerr << "Failed to parse ONNX model: " << sface_path << std::endl;
        return;
    }
    ...
    expected_dim_ = 128;
    std::cout << "Loaded recognition model: " << sface_path
...
FaceRecognitionEngine::FaceRecognitionEngine(const std::string& model_dir, const std::string& db_path)
    : db_path_(db_path), model_dir_(model_dir), expected_dim_(0) {
    ensure_models();
    load_database();
